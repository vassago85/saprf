...
class PrsV14EligibilityRuleset implements EligibilityRuleset
{
    ...
    private function evaluateElg03(User $user): array
    {
        ...
        if (! $residenceProvinceId || ! $club) {
            return [
                'outcome' => SelectionRuleEvaluation::OUTCOME_MANUAL,
                'detail' => ['reason' => 'missing_province_or_club_on_user'],
            ];
        }

        $affiliationMatches = $clubProvinceId && $residenceProvinceId === $clubProvinceId;

        return [
            'outcome' => $affiliationMatches
                ? SelectionRuleEvaluation::OUTCOME_PASS
                : SelectionRuleEvaluation::OUTCOME_FAIL,
            'detail' => [
    ...
    private function evaluateElg05(User $user): array
    {
        $club = $user->club;
        if (! $club) {
            return [
                'outcome' => SelectionRuleEvaluation::OUTCOME_FAIL,
                'detail' => ['reason' => 'no_club_on_user'],
            ];
        }

        return [
            'outcome' => $club->saprf_recognised
                ? SelectionRuleEvaluation::OUTCOME_PASS
                : SelectionRuleEvaluation::OUTCOME_FAIL,
            'detail' => [
                'club_id' => $club->id,
                'club_name' => $club->name,
                'saprf_recognised' => (bool) $club->saprf_recognised,
            ],
        ];
    }
