...
class ImportMembersCommand extends Command
{
    ...
    private function normalizeRow(array $row): array
    {
        ...
        // Scrub obvious junk from the legacy free-text fields.
        $row['sa_id_number'] = $this->sanitizeIdNumber($row['sa_id_number']);
    ...
    /**
     * Drop values that clearly are not an SA ID / passport: GUIDs generated by
     * the legacy system, or anything too long for the column.
     */
    private function sanitizeIdNumber(string $value): string
    {
        $value = trim($value);
        if ($value === '') return '';
        ...
        return mb_strlen($value) > 20 ? '' : $value;
    }
    ...
    private function sanitizePhone(string $value): string
    {
        ...
        $placeholders = ['', 'NO PHONE', 'NO NUMBER', 'NONE', 'N/A', 'NA', 'NO', '-', '--', '0', 'X', 'XX', 'XXX', 'TBC', 'UNKNOWN'];
        return in_array($upper, $placeholders, true) ? '' : $value;
    }
