...
/**
 * Retroactively zero SAPRF + platform fees on any registration that falls
 * inside the pre-billing grace period.
 *
 ...
 * idempotent — rows that are already zeroed are skipped, and the amount that
 * used to sit in `saprf_fee` + `platform_fee` is rolled back into
...
class WaiveRegistrationFeesCommand extends Command
{
    ...
    protected $description = 'Zero SAPRF + platform fees on registrations with registered_at before the cut-off (grace period backfill).';
    ...
    public function handle(SettingsService $settings): int
    {
        ...
        $query = MatchRegistration::query()
            ...
            ->where(function ($q) {
                $q->where('saprf_fee', '>', 0)
                    ->orWhere('platform_fee', '>', 0);
            });

        $summary = $query->clone()
            ->selectRaw('COUNT(*) AS row_count, COALESCE(SUM(saprf_fee), 0) AS saprf_total, COALESCE(SUM(platform_fee), 0) AS platform_total')
            ->first();

        $rows = (int) $summary->row_count;
        $saprfTotal = (float) $summary->saprf_total;
        $platformTotal = (float) $summary->platform_total;

        $this->newLine();
        $this->line(sprintf('%-14s %d registration(s)', 'affected', $rows));
        $this->line(sprintf('%-14s R %s', 'saprf total', number_format($saprfTotal, 2)));
        $this->line(sprintf('%-14s R %s', 'platform total', number_format($platformTotal, 2)));
        $this->line(sprintf('%-14s R %s (added back to md_net_amount)', 'md_net delta', number_format($saprfTotal + $platformTotal, 2)));

        if ($rows === 0) {
            $this->newLine();
            $this->info('Nothing to do — all registrations before the cut-off are already at zero.');
        ...
        // md_net_amount grows by whatever saprf_fee + platform_fee currently
        // are; then both are zeroed. Idempotent because the WHERE clause
        // guarantees at least one of them is > 0.
        $updated = DB::transaction(function () use ($query) {
            return $query->update([
                'md_net_amount' => DB::raw('md_net_amount + saprf_fee + platform_fee'),
                'saprf_fee' => 0,
                'platform_fee' => 0,
                'updated_at' => now(),
            ]);
        ...
        $this->newLine();
        $this->info("Waived fees on {$updated} registration(s).");
