<x-layouts.app :title="'Edit Tier - SAPRF'">
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="font-heading text-3xl font-bold text-stone-900 tracking-tight">Edit Tier: {{ $sponsorTier->name }}</h1>
                <p class="mt-1 text-sm text-stone-500">Update sponsorship tier settings.</p>
            </div>
        ...
        <form method="POST" action="{{ route('sponsor-tiers.update', $sponsorTier) }}" class="rounded-xl border border-stone-200 bg-white shadow-sm p-8 space-y-6 max-w-2xl">
            @csrf
            @method('PUT')
                        ...
                        @foreach ($placements as $key => $label)
                            <label class="flex items-center gap-2">
                                <input type="checkbox" name="placement[]" value="{{ $key }}"
                                    ...
                                    @checked(in_array($key, old('placement', $sponsorTier->placement ?? [])))>
                ...
                <div class="sm:col-span-2">
                    <label class="inline-flex items-center gap-2">
                        ...
                        <input type="checkbox" name="is_active" value="1"
                            class="rounded border-stone-300 text-emerald-600 focus:ring-emerald-500"
                            @checked(old('is_active', $sponsorTier->is_active))>
