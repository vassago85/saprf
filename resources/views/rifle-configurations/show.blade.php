<x-layouts.app :title="$rifle->nickname . ' - SAPRF'">
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <div class="mt-1 flex items-center gap-3">
                    <h1 class="font-heading text-3xl font-bold text-stone-900 tracking-tight">{{ $rifle->nickname }}</h1>
                    @if ($rifle->is_primary)
            ...
            <a href="{{ route('rifle-configurations.edit', $rifle) }}"
            ...
            <div class="rounded-xl border border-stone-200 bg-white shadow-sm p-6 space-y-4">
                ...
                <dl class="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
                    <div>
                        ...
                        <dd class="font-medium text-stone-900">{{ $rifle->make->name ?? '—' }}</dd>
                    </div>
                    <div>
                        <dt class="text-stone-400">Model</dt>
                        <dd class="font-medium text-stone-900">{{ $rifle->model->name ?? '—' }}</dd>
                    </div>
                    <div>
                        <dt class="text-stone-400">Calibre</dt>
                        <dd class="font-medium text-stone-900">{{ $rifle->calibre->name ?? '—' }}</dd>
                    </div>
                    <div>
                        <dt class="text-stone-400">Optic</dt>
                        <dd class="font-medium text-stone-900">{{ $rifle->optic_description ?: '—' }}</dd>
                    </div>
                    <div>
                        <dt class="text-stone-400">Bullet</dt>
                        <dd class="font-medium text-stone-900">{{ $rifle->bullet_description ?: '—' }}</dd>
                    </div>
                    <div>
                        <dt class="text-stone-400">Barrel Length</dt>
                        <dd class="font-medium text-stone-900">{{ $rifle->barrel_length ?: '—' }}</dd>
                    </div>
                    <div>
                        <dt class="text-stone-400">Twist Rate</dt>
                        <dd class="font-medium text-stone-900">{{ $rifle->twist_rate ?: '—' }}</dd>
                    </div>
                </dl>
                @if ($rifle->notes)
                    <div class="border-t border-stone-100 pt-3">
                        <dt class="text-xs text-stone-400 mb-1">Notes</dt>
                        <dd class="text-sm text-stone-700">{{ $rifle->notes }}</dd>
                    </div>
                @endif
            </div>
            ...
            <div class="rounded-xl border border-stone-200 bg-white shadow-sm p-6">
                ...
                <div class="mt-4 grid grid-cols-3 gap-4">
                    <div class="rounded-lg bg-stone-50 p-4 text-center">
                        <p class="text-2xl font-bold text-stone-900">{{ $rifle->scores_count ?? 0 }}</p>
