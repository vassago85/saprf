{{--
    Per-season detail body for the shooter profile — the huge chunk that
    renders each series' breakdown card and per-match contribution table.

    Included from BOTH standings/shooter.blade.php (legacy per-season URL)
    and shooters/profile.blade.php (career hub). Depends on Alpine `active`
    state provided by the outer x-data on the parent view.

    Expected variables (all supplied by ShooterProfileService::season):
      $shooter, $season, $seriesOrder, $summaryBySeries, $scoresBySeries,
      $contributionByMatch, $matchDates, $bestOf.
--}}
