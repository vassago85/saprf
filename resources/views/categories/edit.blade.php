<x-layouts.app :title="'Edit Category'">
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="font-heading text-3xl font-bold text-stone-900 tracking-tight">Edit Category: {{ $category->name }}</h1>
                <p class="mt-1 text-sm text-stone-500">Update category settings.</p>
            </div>
        ...
        <form method="POST" action="{{ route('categories.update', $category) }}" class="rounded-xl border border-stone-200 bg-white shadow-sm p-8 space-y-6 max-w-2xl"
              x-data="{ ageBased: {{ old('is_age_based', $category->is_age_based) ? 'true' : 'false' }} }">
            @csrf
            @method('PUT')
            ...
            <div class="grid sm:grid-cols-2 gap-6">
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="3"
                        class="w-full rounded-lg border-stone-300 text-sm py-2.5 px-3 focus:ring-emerald-500 focus:border-emerald-500">{{ old('description', $category->description) }}</textarea>
                </div>
                ...
                <div class="sm:col-span-2">
                    <label class="inline-flex items-center gap-2">
                        ...
                        <input type="checkbox" name="is_age_based" value="1"
                            ...
                            x-model="ageBased"
                            @checked(old('is_age_based', $category->is_age_based))>
                ...
                <div x-show="ageBased" x-transition class="sm:col-span-2 grid sm:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="number" name="min_age" id="min_age" value="{{ old('min_age', $category->min_age) }}" min="0" max="120"
                            class="w-full rounded-lg border-stone-300 text-sm py-2.5 px-3 focus:ring-emerald-500 focus:border-emerald-500">
                    ...
                    <div>
                        ...
                        <input type="number" name="max_age" id="max_age" value="{{ old('max_age', $category->max_age) }}" min="0" max="120"
                            class="w-full rounded-lg border-stone-300 text-sm py-2.5 px-3 focus:ring-emerald-500 focus:border-emerald-500">
                ...
                <div class="sm:col-span-2">
                    ...
                    <p class="mt-1 text-xs text-stone-400">Uncheck to archive this category. Archived categories won't appear in score entry.</p>
                </div>
            </div>
