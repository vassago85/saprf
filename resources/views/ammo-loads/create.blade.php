<x-layouts.app :title="'Add Ammo Load - ' . $rifleConfiguration->nickname . ' - SAPRF'">
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <a href="{{ route('rifle-configurations.show', $rifleConfiguration) }}" class="text-sm text-emerald-700 font-medium hover:text-emerald-800">&larr; Back to {{ $rifleConfiguration->nickname }}</a>
                <h1 class="mt-1 font-heading text-3xl font-bold text-stone-900 tracking-tight">Add Ammo Load</h1>
                <p class="mt-1 text-sm text-stone-500">
                    For {{ $rifleConfiguration->make?->name }} {{ $rifleConfiguration->model?->name }}
                    @if ($rifleConfiguration->calibre) &middot; {{ $rifleConfiguration->calibre->name }} @endif
                </p>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('ammo-loads.store', $rifleConfiguration) }}"
