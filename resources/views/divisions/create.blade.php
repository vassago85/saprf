<x-layouts.app :title="'Create Division'">
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="font-heading text-3xl font-bold text-stone-900 tracking-tight">Create Division</h1>
                <p class="mt-1 text-sm text-stone-500">Add a new competition division.</p>
            </div>
        ...
        <form method="POST" action="{{ route('divisions.store') }}" class="rounded-xl border border-stone-200 bg-white shadow-sm p-8 space-y-6 max-w-2xl">
            @csrf
            ...
            <div class="grid sm:grid-cols-2 gap-6">
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="3" placeholder="Optional description of this division…"
                        class="w-full rounded-lg border border-stone-300 text-sm py-2.5 px-3 focus:ring-emerald-500 focus:border-emerald-500">{{ old('description') }}</textarea>
                </div>
            </div>
