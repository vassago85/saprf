    ...
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-stone-500">Manage competition divisions and their disciplines.</p>
            </div>
            ...
            <table class="min-w-full divide-y divide-stone-200">
                <thead class="bg-stone-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-stone-500">Discipline</th>
                ...
                <tbody class="divide-y divide-stone-100">
                    @forelse ($divisions as $division)
                        <tr class="{{ !$division->is_active ? 'opacity-50 bg-stone-50' : '' }}">
                            <td class="px-6 py-4 text-sm font-mono font-medium text-stone-900">{{ $division->code }}</td>
                            <td class="px-6 py-4 text-sm text-stone-700">{{ $division->name }}</td>
                            <td class="px-6 py-4">
                                @if ($division->discipline === 'both')
                                    <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold bg-amber-100 text-amber-800 ring-1 ring-inset ring-amber-600/20">Both</span>
                                @else
